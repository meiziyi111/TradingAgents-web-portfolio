...
class TestDeferredReflection:
    ...
    def test_resolve_skips_other_tickers(self, tmp_path):
        ...
        mock_graph._fetch_return_observation = MagicMock(
            return_value=(0.05, 0.02, 5, "2026-01-16")
        )
        TradingAgentsGraph._resolve_pending_entries(mock_graph, "NVDA")
        mock_graph._fetch_return_observation.assert_not_called()
...
class TestPortfolioManagerInjection:
    ...
    def test_pm_rejects_provider_without_structured_output(self):
        """A final decision must never degrade to unvalidated free text."""
        llm = MagicMock()
        llm.with_structured_output.side_effect = NotImplementedError("provider unsupported")
        pm_node = create_portfolio_manager(llm)
        with pytest.raises(RuntimeError, match="requires structured output"):
            pm_node(_make_pm_state())

    def test_pm_returns_typed_decision_artifact_in_state(self):
        captured = {}
        decision = PortfolioDecision(
        ...
        result = pm_node(_make_pm_state())
        assert result["final_trade_decision_structured"] == decision.model_dump(mode="json")
