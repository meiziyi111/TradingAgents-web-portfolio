...
class TestRenderTraderProposal:
    def test_minimal_required_fields(self):
        p = TraderProposal(
            ...
            stop_loss=178.0,
            position_sizing="0% new exposure; maintain current position",
        )
    ...
    def test_optional_fields_omitted_when_absent(self):
        p = TraderProposal(
            ...
            stop_loss=40.0,
            position_sizing="Reduce to 0% after execution",
        )
        md = render_trader_proposal(p)
        assert "Entry Price" not in md
        assert "**Stop Loss**: 40.0" in md
...
class TestTraderAgent:
    ...
    def test_rejects_unstructured_trader_output(self):
        llm = MagicMock()
